...
(`FallbackLlm` in `packages/voicepipe/lib/src/llm/llm.dart`) and the agent
...
`VOICEPIPE_LLM_BASE_URL/API_KEY/MODEL` trio overrides the whole chain.
`VOICEPIPE_LLM_TIMEOUT_SECONDS` (default 45) raises the
per-call LLM timeout — needed for slow free-tier providers (e.g. OpenRouter
`:free` models). `VOICEPIPE_WHISPER_MODEL=tiny|base` picks the STT model.
